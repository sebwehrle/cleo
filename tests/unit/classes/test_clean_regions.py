...
def _write_region_catalog(path: Path, mapping: dict[str, str]) -> None:
    """Write area catalog attrs used by Atlas area resolution."""
...
def _create_region_store_set(
    ...
    *,
    ...
    """Create a minimal area store directory with wind/landscape zarr roots."""
    region_dir = root / "areas" / region_id
...
def test_clean_areas_all_runs(tmp_path: Path) -> None:
...
def test_clean_areas_region_filter_by_name(tmp_path: Path) -> None:
    atlas = Atlas(tmp_path, "AUT", "epsg:3035")
    ...
    deleted = atlas.clean_areas(area="Wien")
...
def test_clean_areas_include_incomplete_flag(tmp_path: Path) -> None:
    atlas = Atlas(tmp_path, "AUT", "epsg:3035")
    ...
    deleted_skip = atlas.clean_areas(include_incomplete=False)
    assert deleted_skip == 0
    assert d_incomplete.exists()

    deleted_drop = atlas.clean_areas(include_incomplete=True)
    assert deleted_drop == 1
    assert not d_incomplete.exists()


def test_clean_areas_older_than(tmp_path: Path) -> None:
    atlas = Atlas(tmp_path, "AUT", "epsg:3035")
    ...
    deleted = atlas.clean_areas(older_than="2021-01-01")
