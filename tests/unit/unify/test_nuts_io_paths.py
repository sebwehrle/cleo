...
def test_read_nuts_area_catalog_raises_when_no_shapefile(tmp_path: Path) -> None:
    atlas = SimpleNamespace(path=tmp_path, country="AUT")
    with pytest.raises(FileNotFoundError, match="NUTS shapefile not found"):
        _read_nuts_area_catalog(atlas)


def test_read_nuts_area_catalog_returns_empty_when_country_has_no_alpha2(
    ...
    rows = _read_nuts_area_catalog(atlas)
    assert rows == []


def test_read_nuts_area_catalog_filters_deduplicates_and_sorts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    shp = tmp_path / "data" / "nuts" / "dummy.shp"
    shp.parent.mkdir(parents=True, exist_ok=True)
    shp.write_text("x", encoding="utf-8")
...
def test_read_nuts_area_catalog_accepts_explicit_path_and_country(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    shp = tmp_path / "data" / "nuts" / "dummy.shp"
    shp.parent.mkdir(parents=True, exist_ok=True)
    ...
    rows = _read_nuts_area_catalog(tmp_path, "AUT")
