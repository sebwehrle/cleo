...
def weibull_pdf_oracle(u: np.ndarray, k: float | np.ndarray, a: float | np.ndarray) -> np.ndarray:
    """
    Closed-form Weibull PDF oracle.
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        z = u / a
        pdf = (k / a) * (z ** (k - 1)) * np.exp(-(z ** k))
    pdf = np.where(u == 0, 0.0, pdf)
    return pdf


def weibull_cdf_oracle(u: float, k: float, a: float) -> float:
    """
    Closed-form Weibull CDF oracle.
